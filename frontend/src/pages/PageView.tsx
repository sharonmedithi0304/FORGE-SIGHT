import { Activity, ArrowRight, ArrowUpRight, FileText, Plus, type LucideIcon } from 'lucide-react'
...
import { ProductionFlowPage, BottleneckAnalysisPage, ProcessDetailPage } from './process/ProcessPages'
import { ProcessIntelligenceWorkspace } from './process/ProcessIntelligenceWorkspace'
...
export function PageView({ page, route }: { page: Page; route: AppRoute }) {
	...
	if (route.path === 'process-intelligence') {
		if (!isDemoLoaded || !data) return <ProcessIntelligenceWorkspaceEmpty loadDemoData={loadDemoData} />
		return <ProcessIntelligenceWorkspace data={data} />
	}
...
function ProcessIntelligenceWorkspaceEmpty({ loadDemoData }: { loadDemoData: () => void }) {
	return <div className="page-view"><div className="page-heading"><div className="heading-icon"><Activity size={21} /></div><div><span className="eyebrow">PROCESS INTELLIGENCE</span><h1>Process Intelligence</h1></div></div><p className="page-description">Load the centralized ForgeSight demo dataset to review batch-specific process conditions.</p><button className="primary-button" type="button" onClick={loadDemoData}>Load demo data <ArrowRight size={15} /></button></div>
}